...
def create_annotation_directory(camera, dir_):
    home_dir ='./'+dir_+"/processedmoose"
...
def write_txt_annotation(frame,cam,directory,dir_):
    ...
    img_path = './'+dir_+"/processedmoose/image_0" + cam + "/data/" + format(frame, '010') + ".png"
    print(img_path)
    if DISTORTED:
      path_type = 'raw'
    else:
      path_type = 'processedmoose'
    annotations_file = './'+dir_+'/3d_annotations.json'; #contains all the annotations
    lidar_path = './'+dir_+"/" + path_type + "/lidar_points/data/" + format(frame, '010') + ".bin" #change this to snowy or not snowy
    calib_path = './'+dir_+'/calibmoose'
...
def write_all_annotations(cam, dir_):
    cam = str(cam)
    directory = create_annotation_directory(cam, dir_)
    img_dir = './'+dir_+"/processedmoose/image_0" + cam + "/data"
    file_names = sorted(os.listdir(img_dir))

    for frame in range(len(file_names)):
        write_txt_annotation(frame,cam,directory, dir_)

# dir is autonomoose_0001 for example
def process_scene(dir_):
    for cam in range(8):
        cam = str(cam)
        write_all_annotations(cam, dir_)
