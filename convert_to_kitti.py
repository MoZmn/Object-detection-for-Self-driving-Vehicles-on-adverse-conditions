...
def convert_kitti(core_number=4):

    #shutil.unpack_archive('2018_03_07.zip', '')

    scenes_list = os.listdir('2018_03_07')
...
#    scenes_list.remove('results.txt')
    ...
    for scene in scenes_list:
        ...
        shutil.copy(os.path.join('2018_03_07', scene)+'/3d_ann.json', os.path.join(root, '3d_annotations.json'))
        calib_files = [item for item in os.listdir('2018_03_07/calib') if item.endswith('yaml')]
        for calib in calib_files:
            shutil.copy(os.path.join('2018_03_07/calib', calib), os.path.join(f'{root}/calibmoose', calib))
        image_folders = [item for item in os.listdir(os.path.join('2018_03_07', scene)+'/labeled') if item.startswith('image')]+['lidar_points']
        for folder in image_folders:
            source = os.path.join('2018_03_07', scene) + '/labeled/'+folder
